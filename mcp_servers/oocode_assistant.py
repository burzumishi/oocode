...
def _tool_read_project_file(args: dict) -> str:
    ...
    directory = args.get("directory", ".")
    max_chars = int(args.get("max_chars", _MAX_OUTPUT))
    root      = Path(directory).expanduser().resolve()
...
def _tool_lint_file(args: dict) -> str:
    ...
    for template in linters:
        ...
        if rc == 0:
            ...
            out_trim = out[:_MAX_OUTPUT]
            if len(out) > _MAX_OUTPUT:
                out_trim += f"\n     … (recortado a {_MAX_OUTPUT} chars)"
...
def _tool_regex_replace(args: dict) -> str:
    ...
    if len(diff_str) > _MAX_OUTPUT:
        diff_str = diff_str[:_MAX_OUTPUT] + "\n... (diff truncado)"
...
def _tool_efm_config_update(args: dict) -> str:
    ...
    """
    try:
        from agent.lsp_client import _ensure_efm_config, _EFM_CONFIG_PATH
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "max_chars": {"type": "integer", "description": f"Chars máximos a devolver (default: {_MAX_OUTPUT})"},
            },
        },
    },
    {
