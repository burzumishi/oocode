...
def _get_prompt(name: str, args: dict) -> list[dict]:
    if name == "pentest_report":
        ...
        text = (
            ...
            f"Alcance: {scope}\n\n"
            f"Hallazgos obtenidos (con nmap_scan, ssl_check, gobuster_run, http_headers, nikto_scan, etc.):\n{findings}\n\n"
            "Estructura el informe con:\n"
            "1. Resumen ejecutivo (severidad global, sistemas críticos afectados)\n"
            "2. Metodología (tools usadas: nmap_scan, ssl_check, gobuster_run, nikto_scan, curl_request)\n"
            "3. Hallazgos por severidad (Crítico/Alto/Medio/Bajo/Informativo)\n"
            "   - Para cada hallazgo: evidencia, impacto, CVSS si aplica, remediación\n"
            "4. Recomendaciones de remediación priorizadas\n"
            "5. Conclusiones\n\n"
            "Usa formato Markdown con tablas para los hallazgos.\n"
            "Si faltan datos, usa estas tools para completarlos:\n"
            "- nmap_scan(target, ports) — puertos y servicios\n"
            "- ssl_check(host, port) — certificados TLS\n"
            "- http_headers(url) — headers de seguridad\n"
            "- whois_lookup(target) — información de dominio"
        )
        ...
        text = (
            ...
            "Proporciona:\n"
            "1. Análisis inicial y clasificación del reto\n"
            "2. Técnicas aplicables según categoría:\n"
            "   - crypto: encode_decode, hash_crack, xor_decode, base_convert, jwt_decode\n"
            "   - web: curl_request, http_headers, gobuster_run, nikto_scan\n"
            "   - forensics: hex_dump, steganography_check, pdf_extract_text\n"
            "   - recon: nmap_scan, dns_enum, whois_lookup, ssl_check, cert_inspect\n"
            "   - pwn/rev: secret_scan, log_analyze, cve_lookup\n"
            "3. Secuencia de tool calls recomendada para resolver el reto\n"
        ...
        text = (
            f"Genera un plan de auditoría de seguridad y ejecútalo para '{stype}'.\n\n"
            f"Contexto: {context}\n\n"
            "FASE 1 — Recopilación de información (ejecuta estas tools):\n"
            "- nmap_scan(target, ports='22,80,443,8080,3306,5432,6379') — puertos abiertos\n"
            "- ssl_check(host) — estado TLS/certificados\n"
            "- http_headers(url) — headers de seguridad (CSP, HSTS, X-Frame, etc.)\n"
            "- whois_lookup(target) — información de dominio/IP\n"
            "- dns_enum(domain) — registros DNS\n\n"
            "FASE 2 — Análisis profundo (según tipo de sistema):\n"
            "- linux_server: fw_audit, ssh_key_audit, sudoers_review, file_integrity_check, log_analyze\n"
            "- web: nikto_scan, gobuster_run, curl_request (endpoints), secret_scan\n"
            "- code_review: secret_scan, log_analyze\n\n"
            "FASE 3 — Informe:\n"
            "Tabla Markdown [Item, Tool usada, Resultado, Riesgo (CRÍTICO/ALTO/MEDIO/BAJO), Remediación]\n"
            "Referencias a estándares: OWASP Top 10, CIS Benchmarks, NIST SP 800-53"
        )
        ...
        text = (
            ...
            "4. Impacto (CVSS score y vector si aplica)\n"
            "5. Verificación con tools disponibles:\n"
            "   - cve_lookup(cve_id) — si es un CVE conocido\n"
            "   - ssl_check(host) — si es vulnerabilidad TLS\n"
            "   - nmap_scan(target) — si requiere verificar versión de servicio\n"
            "   - secret_scan(path) — si puede haber exposición de credenciales\n"
            "6. Mitigaciones y parches disponibles\n"
            "7. Referencias (NVD, advisories, PoC educativo sin código de explotación activo)"
        )
    else:
