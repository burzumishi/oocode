"""Tests de los servidores MCP office / mail / cmdb (split de home_office, v0.4.4).
...
ficheros temporales. Cubre los TRES servidores resultantes del split:
  - office_assistant.py  → documentos O365 (Word/Excel/PPT/PDF/CSV)
  - mail_assistant.py    → email/calendario/notas/contactos
  - cmdb_assistant.py    → inventario IT/CMDB

Los agregados (_TOOLS/_TOOL_FNS/_PROMPTS/_RESOURCES) se prueban como unión de
los tres servidores (66 tools, 19 prompts, 13 resources en total).
...
import mcp_servers.word_assistant as _word
import mcp_servers.excel_assistant as _excel
import mcp_servers.pptx_assistant as _pptx
import mcp_servers.mail_assistant as _mail
import mcp_servers.cmdb_assistant as _cmdb


class _OfficeAgg:
    """Agrega word+excel+pptx en un namespace `office` (el antiguo office_assistant
    se dividió por formato en v0.4.4). Los datos son la unión; las funciones se
    resuelven por fallback entre los tres módulos."""
    _mods = (_word, _excel, _pptx)
    _TOOLS = _word._TOOLS + _excel._TOOLS + _pptx._TOOLS
    _TOOL_FNS = {**_word._TOOL_FNS, **_excel._TOOL_FNS, **_pptx._TOOL_FNS}
    _PROMPTS = {**_word._PROMPTS, **_excel._PROMPTS, **_pptx._PROMPTS}
    _RESOURCES = _word._RESOURCES + _excel._RESOURCES + _pptx._RESOURCES
    _RESOURCE_FNS = {**_word._RESOURCE_FNS, **_excel._RESOURCE_FNS, **_pptx._RESOURCE_FNS}

    def _get_prompt(self, name, args):
        for m in self._mods:
            if name in m._PROMPTS:
                return m._get_prompt(name, args)
        return _word._get_prompt(name, args)
...
# Agregados = unión de los tres servidores (paridad con el antiguo home_office)
_TOOLS       = _office._TOOLS + _mail._TOOLS + _cmdb._TOOLS
_TOOL_FNS    = {**_office._TOOL_FNS, **_mail._TOOL_FNS, **_cmdb._TOOL_FNS}
_PROMPTS     = {**_office._PROMPTS, **_mail._PROMPTS}
_RESOURCES   = _office._RESOURCES + _mail._RESOURCES + _cmdb._RESOURCES
_RESOURCE_FNS = {**_office._RESOURCE_FNS, **_mail._RESOURCE_FNS, **_cmdb._RESOURCE_FNS}


def _get_prompt(name, args):
    """Despacha al servidor dueño del prompt (mail tiene draft_email/meeting_notes)."""
    if name in _mail._PROMPTS:
        return _mail._get_prompt(name, args)
    return _office._get_prompt(name, args)


# Config: TestConfigLoading prueba la config de mail (email + agenda)
_load_config = _mail._load_config
_deep_merge  = _mail._deep_merge
_no_email_config = _mail._no_email_config

# Helpers/tools por servidor (nombres únicos entre módulos)
_build_word_chart_xml = _office._build_word_chart_xml
_tool_doc_create      = _office._tool_doc_create
_read_csv_file        = _office._read_csv_file
_apply_naming         = _office._apply_naming
_parse_oocode_md      = _office._parse_oocode_md
_tool_doc_word_count  = _office._tool_doc_word_count
_tool_csv_analyze     = _office._tool_csv_analyze
_tool_doc_read_template_fields = _office._tool_doc_read_template_fields
_tool_doc_fill_template = _office._tool_doc_fill_template
_tool_doc_list_templates = _office._tool_doc_list_templates
_tool_doc_create_rfc  = _office._tool_doc_create_rfc
_tool_xlsx_fill_range = _office._tool_xlsx_fill_range
_tool_xlsx_append_row = _office._tool_xlsx_append_row
_tool_xlsx_create_report = _office._tool_xlsx_create_report
_tool_project_context_read = _office._tool_project_context_read
_tool_project_init_office = _office._tool_project_init_office
_tool_doc_project_save = _office._tool_doc_project_save
_tool_doc_read        = _office._tool_doc_read
_tool_doc_update_section = _office._tool_doc_update_section
_tool_doc_version_bump = _office._tool_doc_version_bump

_decode_header        = _mail._decode_header
_ics_date_str         = _mail._ics_date_str
_parse_ics_events     = _mail._parse_ics_events
_tool_cal_list        = _mail._tool_cal_list
_tool_cal_add         = _mail._tool_cal_add
_tool_cal_search      = _mail._tool_cal_search
_tool_notes_list      = _mail._tool_notes_list
_tool_notes_search    = _mail._tool_notes_search
_tool_notes_save      = _mail._tool_notes_save
_tool_contact_search  = _mail._tool_contact_search

_tool_cmdb_search     = _cmdb._tool_cmdb_search
_tool_cmdb_update     = _cmdb._tool_cmdb_update
_tool_asset_register_add = _cmdb._tool_asset_register_add
...
class TestConfigLoading:
    ...
    def test_config_has_paths(self):
        ...
        # templates_dir vive ahora en la config de office_assistant
        assert "templates_dir" in _office._load_config()
...
class TestConfigIntegration:
    def test_servers_in_default_config(self):
        from config import DEFAULT_CONFIG
        for key in ("wordAssistant", "excelAssistant", "pptxAssistant",
                    "mailAssistant", "cmdbAssistant"):
            assert key in DEFAULT_CONFIG["mcp"]
            assert "enabled" in DEFAULT_CONFIG["mcp"][key]

    def test_disabled_by_default(self):
        from config import DEFAULT_CONFIG
        for key in ("wordAssistant", "excelAssistant", "pptxAssistant",
                    "mailAssistant", "cmdbAssistant"):
            assert DEFAULT_CONFIG["mcp"][key]["enabled"] is False

    def test_ooconfig_has_fields(self):
        from config import OOConfig
        cfg = OOConfig()
        for attr in ("mcp_word_assistant_enabled", "mcp_excel_assistant_enabled",
                     "mcp_pptx_assistant_enabled", "mcp_mail_assistant_enabled",
                     "mcp_cmdb_assistant_enabled"):
...
def _get_template_tool():
    from mcp_servers.word_assistant import _tool_doc_create_from_template
